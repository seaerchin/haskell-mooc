...
myGcd a 0 = a 
myGcd a b = 
    let (larger, smaller) = (max a b, min a b)
    in myGcd (larger-smaller) smaller
------------------------------------------------------------------------------
...
leftpad s l = 
    let duplicate string n = concat $ replicate n string
        required = l - length s
    in duplicate " " required ++ s
...
smallestDivisor n = minimum [x | x <- [2..n], n `mod` x == 0]
...
biggestPrimeAtMost n = maximum [x |  x <- [1..n], isPrime x]
