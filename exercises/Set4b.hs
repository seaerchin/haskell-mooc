...
maxHelper prev cur = if prev > cur then prev else cur
...
slHelper listVal (summed, l) = (summed + listVal, l + 1)
slStart :: (Double , Int)
slStart = (0, 0)
...
concatStart = []
concatHelper :: [a] -> [a] -> [a]
concatHelper prev cur = prev ++ cur
...
largestHelper x [] = [x]
largestHelper curItem xss@(x: xs)
    | curItem > x = [curItem]
    | curItem == x = curItem : xss
    | otherwise = xss
